...
#pragma unroll 2
  ...
  double * __restrict__ ap  = atry[bid] + n + 1; 
  double const * __restrict__ cgp = cgg[bid] + n + 1; 
...
#pragma unroll 1
	...
	*dbp = dap[0];
      ...
      Alm *= rai;
      double *apo = alphag[bid] + mf1 + threadIdx.x;
  ...
  while(i < nf)
    {
      g = 0;
      n = 0;
      double const * __restrict__ cgp = cg + 1; //cgg, atry
...
#pragma unroll 2
	...
	{ 
	  ...
	  double * __restrict__ CUDA_Plegp = &CUDA_Pleg[m][m][i]; 
#pragma unroll 3
	  for(l = m; l <= lm; l++)
	    {
	      n++;
      ...
      dd *= g;
      int nf1 = nf + 1;
      double * __restrict__ dgp = CUDA_LCC->Dg + (nf1 + i + 1);
...
__device__ double __forceinline__ conv(freq_context * __restrict__ CUDA_LCC, int nc, double * __restrict__ dyda, int bid)
{
  ...
  double * __restrict__ areap = &(Areag[bid][threadIdx.x]);
...
#pragma unroll 2
	  while(xx <= l)
	    {
	      __stwb(alph, __ldg(alph) + wt * dyda[xx]); //ldg
	      alph  += CUDA_BLOCK_DIM;
	    } /* m */
...
#pragma unroll 2
      for(l = 1; l < lastone; l++)
	{
	  ...
	  wt = dyda[l] * sig2iwght;
	  int xx = threadIdx.x;
	  double * __restrict__ alp = alpha + l * mf1 + xx + 1;
#pragma unroll 2
	  while(xx < l)
	    {
	      __stwb(alp, __ldg(alp) + wt * dyda[xx]);
...
#pragma unroll 2
	      while(xx < lastone)
		{
		  ...
		  __stwb(alp, __ldg(alp2) + wt * dyda[xx]);
		  xx += CUDA_BLOCK_DIM;
		  alp  += CUDA_BLOCK_DIM;
		} /* m */
	      //__syncthreads();
	      if(threadIdx.x == 0)
		{
		  ...
		  m = lastone;
		  int * __restrict__ iap = iapp + m + 1;
		  double * __restrict__ alp = alpha + l * mf1 + k + 1;
#pragma unroll 4
		  for(; m <= l; m++)
		    {
		      if(*iap)
			{
			  __stwb(alp, __ldg(alp) + wt * dyda[m]);
			  alp++;
			}
		      iap++;
		    } /* m */
...
#pragma unroll 2
	      ...
	      if(threadIdx.x == 0)
		{
		  ...
		  m = lastone;
		  int * __restrict__ iap = iapp + m + 1;
		  double * __restrict__ alp = alpha + j * mf1 + k + 1;
#pragma unroll 4
		  while(m <= l)
		    {
		      if(*iap)
...
#pragma unroll 
      for(l = 1; l < lastone; l++)
	{
	  ...
	  int xx = threadIdx.x;
	  double * __restrict__ alp = &alpha[l * mf1 + xx];
#pragma unroll 2
	  while(xx <= l)
	    {
	      __stwb(alp, __ldg(alp) + wt * dydap[xx]);
	      xx  += CUDA_BLOCK_DIM;
	      alp += CUDA_BLOCK_DIM;
	    } /* m */
	  //__syncthreads();
...
#pragma unroll 2
	      ...
	      if(threadIdx.x == 0)
		{
		  ...
		  m = lastone;
		  int * __restrict__ iap = iapp + m + 1;
		  double * __restrict__ alp = alpha + l * mf1 + k;
#pragma unroll 4
		  for(; m < l; m++)
		    {
		      if(*iap)
			{
			  __stwb(alp, *alp + wt * dydap[m]);
			  alp++;
			}
		      iap++;
		    } /* m */
...
#pragma unroll 1
      while(j < nf)
	{
	  double dd = __ldca(&areap[j]);
#pragma unroll 3
	  for(int i = 0; i < 3; i++)
	    {
	      double const * __restrict__ norp = CUDA_Nor[i];
	      a[i] += dd * __ldca(&norp[j]);
	    }
	  j += CUDA_BLOCK_DIM;
	}
