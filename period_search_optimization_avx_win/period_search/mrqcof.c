    ...
    for (i = 1; i <= Lcurves; i++)
    {
        ...
        for (jp = 1; jp <= Lpoints[i]; jp++)
        {
            ...
            if (Inrel[i]/* == 1*/)
            {
                ...
                {
                    __m256d avx_dyda = _mm256_load_pd(&dyda[l - 1]), avx_dave = _mm256_loadu_pd(&dave[l]);
            ...
            if (ia[0]) //not relative
            {
                for (jp = 1; jp <= Lpoints[i]; jp++)
                {
                    ...
                    for (l = 1; l <= lastone; l++)  //line of ones
                    {
                        ...
                        for (m = 1; m <= l; m += 4)
                        {
                            __m256d avx_alpha = _mm256_loadu_pd(&alpha[j][k]), avx_dyda = _mm256_loadu_pd(&dyda[m]);
                    ...
                    for (; l <= lastma; l++)  //rest parameters
                    {
                        if (ia[l])
                        {
                            ...
                            {
                                __m256d avx_alpha = _mm256_loadu_pd(&alpha[j][kk]), avx_dyda = _mm256_loadu_pd(&dyda[m]);
                                avx_alpha = _mm256_add_pd(avx_alpha, _mm256_mul_pd(avx_wt, avx_dyda));
            ...
            else //relative ia[0]==0
            {
                for (jp = 1; jp <= Lpoints[i]; jp++)
                {
                    ...
                    for (l = 1; l <= lastone; l++)  //line of ones
                    {
                        ...
                        for (m = 1; m <= l; m += 4)
                        {
                            __m256d avx_alpha = _mm256_load_pd(&alpha[j][k]), avx_dyda = _mm256_loadu_pd(&dyda[m]);
                            avx_alpha = _mm256_add_pd(avx_alpha, _mm256_mul_pd(avx_wt, avx_dyda));
                    ...
                    for (; l <= lastma; l++)  //rest parameters
                    {
                        if (ia[l])
                        {
                            ...
                            for (m = 1; m <= lastone; m += 4)
                            {
                                __m256d avx_alpha = _mm256_load_pd(&alpha[j][kk]), avx_dyda = _mm256_loadu_pd(&dyda[m]);
                                avx_alpha = _mm256_add_pd(avx_alpha, _mm256_mul_pd(avx_wt, avx_dyda));
