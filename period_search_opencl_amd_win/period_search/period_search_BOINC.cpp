...
int main(int argc, char **argv) {
    ...
    while ((new_conw != 1) && ((conw_r * escl * escl) < 10.0))
    {
        ...
        freq_start = 1.0 / per_start;
        freq_end = 1.0 / per_end;
        freq_step = 0.5 / (jd_max - jd_min) / 24.0 * per_step_coef;
        ...
        if (boinc_is_standalone())
        {
            printf("(n) \tFrequency\tPeriod\tcg[Ncoef + 3]\n");
        }
    ...
    for (n; n <= count; n++)
    {
        ...
        /* output file */
        if (n == 1)
            out.printf("%.8f  %.6f  %.6f %4.1f %4.0f %4.0f\n", 24.0 * per_best, dev_best, dev_best * dev_best * (ndata - 3), conw_r * escl * escl, la_best, be_best);
        else
        ...
        {
            if (n == 1)
                printf("%.8f  %.6f  %.6f %4.1f %4.0f %4.0f  done %.2f\n", 24.0 * per_best, dev_best, dev_best * dev_best * (ndata - 3), conw_r * escl * escl, la_best, be_best, fraction_done);
            else
