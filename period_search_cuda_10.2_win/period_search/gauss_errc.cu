...
//	__shared__ int indxc[MAX_N_PAR+1],indxr[MAX_N_PAR+1],ipiv[MAX_N_PAR+1];
	...
	double big,dum,temp;
	__shared__ int icol,sh_icol[CUDA_BLOCK_DIM],sh_irow[CUDA_BLOCK_DIM];
	__shared__ double pivinv,sh_big[CUDA_BLOCK_DIM];
...
/*					        deallocate_vector((void *) ipiv);
		...
		if (threadIdx.x==0)
		{
			...
			++((*CUDA_LCC).ipiv[icol]);
			if (irow != icol) {
				for (l=1;l<=n;l++) SWAP((*CUDA_LCC).covar[irow*(CUDA_mfit1)+l],(*CUDA_LCC).covar[icol*(CUDA_mfit1)+l])
				SWAP(b[irow],b[icol])
			}
	...
	/*					    deallocate_vector((void *) ipiv);
		...
		for (l=brtmpl;l<=brtmph;l++) (*CUDA_LCC).covar[icol*(CUDA_mfit1)+l] *= pivinv;
		__syncthreads();
