...
int main(int argc, char **argv) {
	...
	double jd_0, jd_00, conw, conw_r, a0 = 1.05, b0 = 1.00, c0 = 0.95, a, b, c_axis,
		...
		**aalpha, chck[4], *sig,
		*tim, *al,
		a_lamda_start, a_lamda_incr, phi_0,
	...
	for (int ii = 0; ii < argc; ii++) {
		if (openClDevice < 0 && strcmp(argv[ii], "--device") == 0 && ii + 1 < argc)
			openClDevice = atoi(argv[++ii]);
	}
	if (openClDevice < 0)
		openClDevice = 0;

	//retval = ClPrepare(openClDevice, beta_pole, lambda_pole, par, cl, a_lamda_start, a_lamda_incr, ee, ee0, tim, phi_0, checkpoint_exists, ndata);
	if (!retval)
	{
		fflush(stderr);
		exit(2);
