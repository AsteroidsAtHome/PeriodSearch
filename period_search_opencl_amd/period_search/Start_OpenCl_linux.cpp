	...
	cl_double ee[][3], cl_double ee0[][3], cl_double* tim, cl_double Phi_0, cl_int checkex, cl_int ndata)
{
	...
	cl_platform_id platforms[num_platforms_available];
	...
	size_t nameSize;
	err_num = clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(strBufSize), NULL, &nameSize);
	...
	err_num = clGetDeviceInfo(device, CL_DEVICE_VENDOR, sizeof(strBufSize), &deviceVendor, NULL);
	err_num = clGetDeviceInfo(device, CL_DRIVER_VERSION, sizeof(strBufSize), &driverVersion, NULL);
	
	size_t valueSize;
    clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, 0, NULL, &valueSize);
    auto openClVersion = (char*) malloc(valueSize);
    clGetDeviceInfo(device, CL_DEVICE_OPENCL_C_VERSION, valueSize, openClVersion, NULL);

	clGetDeviceInfo(device, CL_DEVICE_VERSION, 0, NULL, &valueSize);
    auto clDeviceVersion = (char*) malloc(valueSize);
    clGetDeviceInfo(device, CL_DEVICE_VERSION, valueSize, clDeviceVersion, NULL);

	// cl_device_exec_capabilities
	char clDeviceExtensionCapabilities[strBufSize];
	err_num = clGetDeviceInfo(device, CL_DEVICE_EXECUTION_CAPABILITIES, sizeof(strBufSize), &clDeviceExtensionCapabilities, NULL);
	...
	// char deviceExtensions[strBufSize];
	// err_num = clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, sizeof(strBufSize), &deviceExtensions, NULL);

	clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, 0, NULL, &valueSize);
    auto deviceExtensions = (char*) malloc(valueSize);
    clGetDeviceInfo(device, CL_DEVICE_EXTENSIONS, valueSize, deviceExtensions, NULL);
	...
	size_t devWorkItemSizes[devMaxWorkItemDims];
	...
	cerr << "CL_DEVICE_GLOBAL_MEM_SIZE: " << clDeviceGlobalMemSize << " B" << endl;
	...
	std::ofstream out(kernelSourceFile, ios::out | ios::binary);
	out << kernel_code;
	out.close();
#endif
	...
	try
	{
		...
		err_num = clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, bufSize, buildlog, NULL);
		cl_build_status buildStatus;
...
#pragma endregion
	...
	if(!strcmp(vendor, "Mesa"))
	{
		...
		cerr << "Prefered kernel work group size [Mesa]: " << preferedWGS << " | " << 2 * preferedWGS << endl;
...
#pragma endregion
	...
	for (n = 1; n <= max_test_periods; n += CUDA_grid_dim_precalc)
	{
		...
		err = clEnqueueNDRangeKernel(queue, kernelCalculatePrepare, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
		clFinish(queue);
...
//#ifndef INTEL
			...
			err = clEnqueueNDRangeKernel(queue, kernelCalculatePreparePole, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
			clFinish(queue);

			// queue.enqueueReadBuffer(CUDA_CC2, CL_BLOCKING, 0, faSize, Fb);
			// auto pFb = queue.enqueueMapBuffer(CUDA_CC2, CL_BLOCKING, CL_MAP_READ | CL_MAP_WRITE, 0, faSize);
			// queue.flush();
			...
			while (!theEnd)
			{
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Begin, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1Start, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue);
				//queue.enqueueBarrierWithWaitList(); // TEST
				for (iC = 1; iC < l_curves; iC++)
				{
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1Matrix, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1Curve1, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1Curve2, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
				}
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1Curve1Last, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1End, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqmin1End, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue);
				//queue.enqueueBarrierWithWaitList(); // TEST
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2Start, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue);
				...
				for (iC = 1; iC < l_curves; iC++)
				{
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2Matrix, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2Curve1, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
					...
					err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2Curve2, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
					clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2Curve1Last, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2End, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqmin2End, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter2, 1, NULL, &totalWorkItems, &local, 0, NULL, NULL);
				clFinish(queue); // ***
...
//#ifndef INTEL
			...
			err = clEnqueueNDRangeKernel(queue, kernelCalculateFinishPole, 1, NULL, &CUDA_grid_dim_precalc, &sLocal, 0, NULL, NULL);
			clFinish(queue);
...
//	}
	...
	for (n = n_start_from; n <= n_max; n += CUDA_grid_dim)
	{
		...
		err = clSetKernelArg(kernelCalculatePrepare, 6, sizeof(n), &n);
		err = clEnqueueNDRangeKernel(queue, kernelCalculatePrepare, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
		clFinish(queue);
		...
		for (m = 1; m <= N_POLES; m++)
		{
			...
			err = clEnqueueWriteBuffer(queue, CUDA_End, CL_BLOCKING, 0, sizeof(theEnd), &theEnd, 0, NULL, NULL);
			err = clSetKernelArg(kernelCalculatePreparePole, 6, sizeof(m), &m);
			err = clEnqueueNDRangeKernel(queue, kernelCalculatePreparePole, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
			clFinish(queue);
			...
			while (!theEnd)
			{
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Begin, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				// //mrqcof
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof1End, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqcof2End, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
				clFinish(queue);
				//mrqcof
				...
				err = clEnqueueNDRangeKernel(queue, kernelCalculateIter1Mrqmin2End, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
				clFinish(queue);
			...
			err = clEnqueueNDRangeKernel(queue, kernelCalculateFinishPole, 1, NULL, &CUDA_grid_dim, &sLocal, 0, NULL, NULL);
			clFinish(queue);
