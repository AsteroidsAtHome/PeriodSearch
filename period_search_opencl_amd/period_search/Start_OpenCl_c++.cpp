...
cl_int ClPrepare(cl_int deviceId,
                 cl_double *beta_pole,
                 cl_double *lambda_pole,
                 cl_double *par,
                 cl_double lcoef,
                 ...
                 cl_int ndata)
{
    ...
    {
        name = (*iter).getInfo<CL_PLATFORM_NAME>();
    ...
    auto cl_dev = deviceIds[deviceId];
    auto device = cl::Device(cl_dev);
    ...
    // NOTE: We need just a single device inside the vector. The choosen one;
    //devices.push_back(cl::Device(deviceIds[deviceId]));

    // Create an OpenCL context
    cl_context_properties properties[] = {CL_CONTEXT_PLATFORM, (cl_context_properties)platform, 0};
    cl_context clContext;
    cl_device_id singleDevId[1]{deviceIds[deviceId]};

    clContext = clCreateContext(properties, 1, singleDevId, NULL, NULL, NULL);
...
#if defined AMD
    char deviceBoardName[128] = "";
    clGetDeviceInfo(deviceIds[deviceId], CL_DEVICE_BOARD_NAME_AMD, sizeof(deviceBoardName), (void *)deviceBoardName, NULL);
#endif
    ...
    auto globalMemory = clGlobalMemory / 1048576;


    std::cerr << "Device OpenCL supported version: " << openClVersion << " | " << clDeviceVersion << endl;
    std::cerr << "OpenCL device Id : " << deviceId << endl;
    string memSufix = "MB";
...
#if defined AMD
    std::cerr << "OpenCl device name: " << deviceBoardName << " (" << deviceName << ") " << globalMemory << memSufix
              << endl;
#elif
    std::cerr << "OpenCl device name: " << deviceName << " " << globalMemory << "MB" << endl;
#endif

    std::cerr << "Device driver: " << driverVersion << std::endl;
    ...
    std::cerr << "CL_DEVICE_MAX_WORK_ITEM_SIZES: " << devMaxWorkItemSizes[0] << " | " << devMaxWorkItemSizes[1] << " | "
              << devMaxWorkItemSizes[2] << endl;
#endif
    ...
    if (doesNotSupportsFp64)
    {
        fprintf(
            stderr,
            "Double precision floating point not supported by OpenCL implementation on current device34. Exiting...\n");
        return (1);
    }

    /*string testKernelSrc = "__kernel void k(){}";
    cl::Kernel testKernel;
    cl::Program testProgram = cl::Program();
    testKernel = cl::Kernel(program, "k", &err);*/
    //auto t = testKernel.getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(device);
...
#pragma region Load kernel files
    ...
    if (!kernelExist || readsource)
    {
        ...
        //cl::Program binProgram = cl::Program(context, ocl_src_kernelSource, true, &perr);
        cl_program clBinProgram = clCreateProgramWithSource(context(), 1, (const char**)&ocl_src_kernelSource, NULL, &perr);
        cl::Program binProgram = cl::Program(clBinProgram);
        if (perr != CL_SUCCESS)
        {
            cerr << "Error: Failed to create compute program! " << cl_error_to_str(perr) << " (" << perr << ")"
                 << endl;
            return EXIT_FAILURE;
        }
#else
...
#pragma endregion
        ...
        //perr = binProgram.build(devices, "-Werror");//        -cl - std = CL1 .2 ");
        char options[]{ "-Werror" };
        perr = clBuildProgram(clBinProgram, 1, &device(), options, NULL, NULL);

        /*if (perr != CL_SUCCESS)
        {
            cerr << "Error: Failed to build binary program! " << cl_error_to_str(perr) << " (" << perr << ")" << endl;
            return EXIT_FAILURE;
        }*/
#elif defined(NVIDIA)
        ...
        //std::string buildlog = binProgram.getBuildInfo<CL_PROGRAM_BUILD_LOG>(devices[0]);
        //cl_build_status buildStatus = binProgram.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(devices[0]);
        //if (buildStatus == 0)
        //{
        //    buildlog.append("OK ");
        //}
        //std::cerr << "Binary build log for " << deviceName << ":" << std::endl
        //          << buildlog << " (" << buildStatus << ")" << std::endl;

        size_t len;
        clGetProgramBuildInfo(clBinProgram, device(), CL_PROGRAM_BUILD_LOG, 0, NULL, &len);
        char* buildlog = (char*)calloc(len, sizeof(char));
        clGetProgramBuildInfo(clBinProgram, device(), CL_PROGRAM_BUILD_LOG, len, buildlog, NULL);

        cl_build_status buildStatus;
        clGetProgramBuildInfo(clBinProgram, device(), CL_PROGRAM_BUILD_STATUS, sizeof(cl_build_status), &buildStatus, NULL);
        ...
        program.build(
            devices,
            "-D AMD -cl-std=CL1.2"); //, "-g -x cl -cl-std=CL1.2 -Werror"); // "-Werror" "-w" "-cl-std=CL1.2" " -fno-bin-amdil"
#elif defined(NVIDIA)
        ...
        std::string buildOptions = program.getBuildInfo<CL_PROGRAM_BUILD_OPTIONS>(devices[0]);
        ...
        std::cerr << "Build log for " << deviceName << ":" << std::endl
                  << buildlog << " (" << buildStatus << ")" << std::endl;
        //}
    }
    ...
    catch (cl::Error &e)
    {
        if (e.err() == CL_BUILD_PROGRAM_FAILURE)
        {
            ...
            //{
                ...
                cl_build_status status1 = program.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(device);
        ...
        else
        {
            ...
            //{
                std::string name = device.getInfo<CL_DEVICE_NAME>();
                std::string deviceDriver = device.getInfo<CL_DRIVER_VERSION>();
                std::string buildlog = program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(device);
                std::string buildOptions = program.getBuildInfo<CL_PROGRAM_BUILD_OPTIONS>(device);
                cl_build_status buildStatus = program.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(device);
...
#pragma region Kernel creation
    ...
    {
        cerr << "Error creating kernel: \"" << cl_error_to_str(e.err()) << "\"(" << e.err() << ") - " << e.what()
             << " | " << cl_error_to_str(kerr) << " (" << kerr << ")" << std::endl;
        cout << "Error while creating kernel. Check stderr.txt for details." << endl;
        return (4);
    }
#pragma endregion

    auto preferedWGS = kernelCalculateIter1Mrqmin1End.getWorkGroupInfo<CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE>(device, &kerr);
    cerr << "Kernel prefered work group size multiple: " << preferedWGS << endl;
    CUDA_grid_dim = preferedWGS * 2;
...
cl_int ClPrecalc(cl_double freq_start,
                 cl_double freq_end,
                 cl_double freq_step,
                 cl_double stop_condition,
                 cl_int n_iter_min,
                 cl_double *conw_r,
                 cl_int ndata,
                 cl_int *ia,
                 ...
                 int Ncoef)
{
...
#elif defined AMD
    auto cgFirst = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(cl_double) * (MAX_N_PAR + 1), cg_first, err);
#elif defined NVIDIA
    ...
    //auto pcc = (mfreq_context *)_aligned_malloc(pccSize, 256);
    ...
    void *pFa = queue.enqueueMapBuffer(CUDA_CC, CL_BLOCKING, CL_MAP_WRITE, 0, faSize, NULL, NULL, err);
    memcpy(pFa, Fa, faSize);
    queue.enqueueUnmapMemObject(CUDA_CC, pFa); //, 0, NULL, NULL);
    queue.flush();
#elif defined NVIDIA
...
#if defined(INTEL)
        fres = (freq_result *)
                   queue.enqueueMapBuffer(CUDA_FR, CL_BLOCKING, CL_MAP_READ, 0, frOptimizedSize, NULL, NULL, err);
        queue.finish();
#elif defined AMD
...
#if defined _DEBUG
    fprintf(stderr, "ave_dark_facet: %10.17f\n", ave_dark_facet);
    fprintf(stderr, "conw_r:         %10.17f\n", *conw_r);
#endif

    return 0;
}

int CUDAStart(int n_start_from,
              double freq_start,
              double freq_end,
              double freq_step,
...
#if defined(INTEL)
    auto cgFirst =
        cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(double) * (MAX_N_PAR + 1), cg_first, err);
#else
    auto cgFirst = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(double) * (MAX_N_PAR + 1), cg_first, err);
    queue.enqueueWriteBuffer(cgFirst, CL_TRUE, 0, sizeof(double) * (MAX_N_PAR + 1), cg_first);
#endif
...
#else
    auto CUDA_End = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof(int), &theEnd, err);
    queue.enqueueWriteBuffer(CUDA_End, CL_BLOCKING, 0, sizeof(cl_int), &theEnd);
#endif
...
#elif defined AMD
    //int frSize = CUDA_grid_dim * sizeof(freq_result);
    int frSize = ((sizeof(freq_result) * CUDA_grid_dim - 1) / 64 + 1) * 64;
    void *memIn = (void *)_aligned_malloc(frSize, 256);
    auto CUDA_FR = cl::Buffer(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, frSize, memIn, err);
    void *pfr;
#elif NVIDIA
...
#pragma region Kernels
    ...
    kernelCalculateIter1Mrqcof1Curve1Last =
        cl::Kernel(program, string("ClCalculateIter1Mrqcof1Curve1Last").c_str(), &kerr);
    kernelCalculateIter1Mrqcof1End = cl::Kernel(program, string("ClCalculateIter1Mrqcof1End").c_str(), &kerr);
    ...
    kernelCalculateIter1Mrqcof2Curve1Last =
        cl::Kernel(program, string("ClCalculateIter1Mrqcof2Curve1Last").c_str(), &kerr);
    kernelCalculateIter1Mrqcof2End = cl::Kernel(program, "ClCalculateIter1Mrqcof2End", &kerr);
    kernelCalculateIter1Mrqmin2End = cl::Kernel(program, "ClCalculateIter1Mrqmin2End", &kerr);
    kernelCalculateIter2 = cl::Kernel(program, "ClCalculateIter2", &kerr);
    kernelCalculateFinishPole = cl::Kernel(program, "ClCalculateFinishPole", &kerr);
...
#pragma endregion
    ...
    for (n = n_start_from; n <= n_max; n += CUDA_grid_dim)
    {
        ...
        kernelCalculatePrepare.setArg(
            6,
            sizeof(n),
            &n); // NOTE: CudaCalculatePrepare << <CUDA_grid_dim, 1 >> > (n, n_max, freq_start, freq_step);
        ...
        for (m = 1; m <= N_POLES; m++)
        {
            ...
            queue.enqueueWriteBuffer(
                ...
                0,
                sizeof(cl_int),
                &theEnd); /// <<<<<<<<<<<<<<<	// CudaCalculatePreparePole << <CUDA_grid_dim, 1 >> > (m);
            queue.enqueueNDRangeKernel(kernelCalculatePreparePole,
                                       cl::NDRange(),
                                       cl::NDRange(CUDA_grid_dim),
                                       cl::NDRange(1));
            ...
            while (!theEnd)
            {
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Begin,
                                           cl::NDRange(),
                                           cl::NDRange(CUDA_grid_dim),
                                           cl::NDRange(1));
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1Start,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));

                for (iC = 1; iC < l_curves; iC++)
                {
                    kernelCalculateIter1Mrqcof1Matrix.setArg(
                        2,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof1Matrix << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1Matrix,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));

                    kernelCalculateIter1Mrqcof1Curve1.setArg(2, sizeof(in_rel[iC]), &(in_rel[iC]));
                    kernelCalculateIter1Mrqcof1Curve1.setArg(
                        3,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof1Curve1 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[iC], l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1Curve1,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));

                    kernelCalculateIter1Mrqcof1Curve2.setArg(2, sizeof(in_rel[iC]), &(in_rel[iC]));
                    kernelCalculateIter1Mrqcof1Curve2.setArg(
                        3,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof1Curve2 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[iC], l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1Curve2,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));
                }
                ...
                kernelCalculateIter1Mrqcof1Curve1Last.setArg(2, sizeof(in_rel[l_curves]), &(in_rel[l_curves]));
                kernelCalculateIter1Mrqcof1Curve1Last.setArg(
                    3,
                    sizeof(l_points[l_curves]),
                    &(l_points
                          [l_curves])); //CudaCalculateIter1Mrqcof1Curve1Last << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[l_curves], l_points[l_curves]);
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1Curve1Last,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof1End,
                                           cl::NDRange(),
                                           cl::NDRange(CUDA_grid_dim),
                                           cl::NDRange(1));
                //mrqcof
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqmin1End,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));

                //CudaCalculateIter1Mrqcof2Start << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > ();
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Start,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));
                //clFinish(queue());
                ...
                for (iC = 1; iC < l_curves; iC++)
                {
                    kernelCalculateIter1Mrqcof2Matrix.setArg(
                        2,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof2Matrix << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Matrix,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));

                    kernelCalculateIter1Mrqcof2Curve1.setArg(2, sizeof(in_rel[iC]), &(in_rel[iC]));
                    kernelCalculateIter1Mrqcof2Curve1.setArg(
                        3,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof2Curve1 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[iC], l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Curve1,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));

                    kernelCalculateIter1Mrqcof2Curve2.setArg(2, sizeof(in_rel[iC]), &(in_rel[iC]));
                    kernelCalculateIter1Mrqcof2Curve2.setArg(
                        3,
                        sizeof(l_points[iC]),
                        &(l_points
                              [iC])); //CudaCalculateIter1Mrqcof2Curve2 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[iC], l_points[iC]);
                    queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Curve2,
                                               cl::NDRange(),
                                               cl::NDRange(totalWorkItems),
                                               cl::NDRange(BLOCK_DIM));
                }
                ...
                kernelCalculateIter1Mrqcof2Curve1Last.setArg(2, sizeof(in_rel[l_curves]), &in_rel[l_curves]);
                kernelCalculateIter1Mrqcof2Curve1Last.setArg(
                    3,
                    sizeof(l_points[l_curves]),
                    &l_points
                        [l_curves]); //CudaCalculateIter1Mrqcof2Curve1Last << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[l_curves], l_points[l_curves]);
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Curve1Last,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));
                ...
                kernelCalculateIter1Mrqcof2Curve2.setArg(
                    ...
                    sizeof(l_points[l_curves]),
                    &l_points
                        [l_curves]); //CudaCalculateIter1Mrqcof2Curve2 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > (in_rel[l_curves], l_points[l_curves]);
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2Curve2,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqcof2End,
                                           cl::NDRange(),
                                           cl::NDRange(CUDA_grid_dim),
                                           cl::NDRange(1));
                //mrqcof
                ...
                queue.enqueueNDRangeKernel(kernelCalculateIter1Mrqmin2End,
                                           cl::NDRange(),
                                           cl::NDRange(CUDA_grid_dim),
                                           cl::NDRange(1));

                //CudaCalculateIter2 << <CUDA_grid_dim, CUDA_BLOCK_DIM >> > ();
                queue.enqueueNDRangeKernel(kernelCalculateIter2,
                                           cl::NDRange(),
                                           cl::NDRange(totalWorkItems),
                                           cl::NDRange(BLOCK_DIM));
            ...
            queue.enqueueNDRangeKernel(kernelCalculateFinishPole,
                                       cl::NDRange(),
                                       cl::NDRange(CUDA_grid_dim),
                                       cl::NDRange(1));
            ...
            if (res[m - 1].isReported == 1)
            {
                ...
                if (n == 1 && m == 1)
                {
                    ...
                    mf.printf("%.8f  %.6f  %.6f %4.1f %4.0f %4.0f\n",
                              ...
                              res[m - 1].dev_best_x2,
                              conw_r * escl * escl,
                              round(res[m - 1].la_best),
                              round(res[m - 1].be_best));
                }
                else
                {
        ...
        if (boinc_time_to_checkpoint() || boinc_is_standalone())
        {
            ...
            {
                fprintf(stderr,
                        "%s APP: period_search checkpoint failed %d\n",
